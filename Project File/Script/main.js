...
function normalizeInstallerPreferences(preferences) {
  ...
  if (
    typeof preferences.username === "string" &&
  ...
  if (
    preferences.pdfQualityMode === "light" ||
    preferences.pdfQualityMode === "original"
  ) {
...
function applyInstallerPreferences() {
  ...
  try {
    const preferences = normalizeInstallerPreferences(
      parseInstallerPreferences(
        readTextFileWithDetectedEncoding(preferencesPath),
      ),
    );
    ...
    if (fs.existsSync(configPath)) {
      ...
      const collectBookPaths = (items = []) => {
        items.forEach((item) => {
          ...
          if (item.structureType === "series" && item.path)
...
function isKnownAllowedPath(targetPath) {
  ...
  if (allowedPickedFiles.has(lower) || allowedPickedDirs.has(lower))
    return true;
...
function getCreateFolderBasePath(requestedBasePath) {
  ...
  try {
    const data = readUserConfig();
    const customFolders = Array.isArray(data?.customFolders)
      ? data.customFolders
      : [];
    ...
    return allowedRoots.some(
      (rootPath) => rootPath.toLowerCase() === normalizedBasePath.toLowerCase(),
    )
      ? normalizedBasePath
      : null;
  } catch {
...
function createWindow() {
  ...
  mainWindow.webContents.on(
    ...
    (event, errorCode, errorDescription, validatedURL) => {
      console.error(
        `Gagal memuat ${validatedURL}: ${errorCode} ${errorDescription}`,
      );
    },
  );
...
ipcMain.handle("dialog:openFile", async () => {
  const { canceled, filePaths } = await dialog.showOpenDialog({
    ...
    filters: [
      {
        name: "Documents",
        extensions: ["pdf", "epub", "cbz", "zip", "cbr", "txt", "md", "docx"],
      },
    ],
  });
  if (canceled) {
...
ipcMain.handle("data:backup", async () => {
  ...
  try {
    if (!fs.existsSync(sourcePath)) {
      return {
        success: false,
        message: "Belum ada data aplikasi yang bisa dibackup.",
      };
    }
    ...
    const { canceled, filePath } = await dialog.showSaveDialog({
      ...
      defaultPath: path.join(
        ...
        `KeiYomi_Backup_${dateStamp}.json`,
      ),
      filters: [{ name: "KeiYomi Backup", extensions: ["json"] }],
    });
...
ipcMain.handle("data:restore", async () => {
  ...
  try {
    ...
    if (parsed.library !== undefined && !Array.isArray(parsed.library)) {
      return {
        success: false,
        message: "Format library pada file backup tidak valid.",
      };
    }
    ...
    if (parsed.history !== undefined && !Array.isArray(parsed.history)) {
      return {
        success: false,
        message: "Format history pada file backup tidak valid.",
      };
    }
...
async function extractCbrImages(filePath) {
  if (
    !isAllowedReadableFile(filePath, allowedDocumentExts) ||
    path.extname(filePath).toLowerCase() !== ".cbr"
  ) {
  ...
  const [archiveBuffer, wasmBuffer] = await Promise.all([
    ...
    fs.promises.readFile(
      path.join(__dirname, "../vendor/node-unrar-js/js/unrar.wasm"),
    ),
  ]);
  ...
  const extracted = extractor.extract({
    files: (fileHeader) => {
      if (
        fileHeader.flags &&
        (fileHeader.flags.directory || fileHeader.flags.encrypted)
      )
        return false;
      return cbrImageExts.has(
  ...
  const images = Array.from(extracted.files)
    .filter(
      (file) =>
        file.extraction &&
        cbrImageExts.has(
          path.extname(file.fileHeader.name || "").toLowerCase(),
        ),
    )
    .map((file) => ({
  ...
  try {
    ...
    if (data.cover && fs.existsSync(data.cover)) {
      if (!isAllowedReadableFile(data.cover, allowedImageExts)) {
        return {
          success: false,
          message: "Akses gambar sampul tidak diizinkan.",
        };
      }
        ...
        // KASUS 2: Folder Khusus (Structured/Manga)
          ...
          if (fs.existsSync(infoPath)) {
            try {
              ...
              if (
                detectedCover &&
                !fs.existsSync(path.join(fullPath, detectedCover))
              ) {
...
ipcMain.handle("updater:check", async () => {
  try {
    ...
    const remoteInfo = await new Promise((resolve, reject) => {
      ...
      request.on("response", (response) => {
        if (response.statusCode !== 200) {
          reject(
            new Error(`Gagal akses update.json: HTTP ${response.statusCode}`),
          );
          return;
        }
    ...
    const releaseUrl = isAllowedReleaseUrl(remoteInfo.releaseUrl)
